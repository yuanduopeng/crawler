...
def crawler_mail(inc):
    ...
    print("Searching Ultimate stock status ... ")
    (Ult_nam, Ult_sts) = getData(UltimateUlr)
    BikeName = Aer_nam + Ult_nam
    BikeStock = Aer_sts + Ult_sts
    # 2.发送邮件
    # 2.1 确认是否要发送邮件
    SendEmailOrNot = 0
    for sts in BikeStock:
        if sts == "available":
            SendEmailOrNot = 1
    # 2.2 拼接邮件消息内容
    MassageContent = ""
    for i in range(1,len(BikeStock)):
        if BikeStock[i] == "available":
            MassageContent = MassageContent + BikeName[i] + ": " + BikeStock[i] + "\n"
    print(MassageContent)
    # 2.3 发出邮件
    if SendEmailOrNot == 1:
        print("Bike is available, so E-mail will be sent.")
        sendEmail(MassageContent)
